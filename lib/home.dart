...
class _HomeState extends State<Home> {
  ...
  // --- LOGIQUE DE TRI STRICTE (Aucun mélange de mots-clés) ---
  String _detectRegion(Map<String, dynamic> item) {
    final String rawRegion = item['region'] ?? '';
    final String rawSub = item['sub_folder'] ?? '';

    // 1. Confiance absolue aux clés principales si elles sont correctes
    ...
    // 2. Rattrapage basé UNIQUEMENT sur le dossier exact (pas sur le nom du lieu)
  ...
  String _detectSubFolder(Map<String, dynamic> item, String region) {
    ...
    // Tolérance pour l'accent : on convertit "Desert" sans accent vers "Désert" pour que ça rentre dans le bon dossier visuel
    if (rawSub == "Desert") return "Désert";

    // 1. Confiance absolue au sous-dossier s'il existe dans notre liste stricte
    ...
    // 2. Sécurité : Forcer le bon dossier en fonction de la région
                      ...
                      children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text("Guide officiel", style: GoogleFonts.montserrat(color: _accentGold, fontSize: 10, fontWeight: FontWeight.bold, letterSpacing: 3)),
                              const SizedBox(height: 2),
                              RichText(
                                text: TextSpan(
                                  style: GoogleFonts.playfairDisplay(fontSize: 24, fontWeight: FontWeight.bold, color: Colors.white),
                                  children: const [TextSpan(text: "Égypte"), TextSpan(text: " Eternelle", style: TextStyle(fontStyle: FontStyle.italic))],
                                ),
                              ),
                            ],
                          ),
                    ...
                    Row(
                      ...
                      children: [
                        Container(
                          ...
                          child: Text('Cliquez pour découvrir', style: GoogleFonts.montserrat(color: themeColor, fontSize: 11, fontWeight: FontWeight.bold)),
                        ),
  ...
  List<Widget> _buildFavoritesFolders(Map<String, Map<String, List<Map<String, dynamic>>>> grouped) {
    ...
    final List<Map<String, dynamic>> folderConfig = [
      {"title": "Villes Historiques", "color": _accentGold, "icon": Icons.location_city_rounded},
      {"title": "Nature & Aventure", "color": Colors.tealAccent, "icon": Icons.landscape_rounded},
      {"title": "Mer Rouge", "color": Colors.blueAccent, "icon": Icons.waves_rounded},
      {"title": "Infos Pratiques", "color": Colors.redAccent, "icon": Icons.info_outline_rounded},
    ];
    ...
    for (var folder in folderConfig) {
      ...
      if (mainTitle == "Infos Pratiques") {
        subFolderLabel = "$totalItemsInFolder ${totalItemsInFolder <= 1 ? 'infos' : 'infos'}";
      } else {
        subFolderLabel = "$totalItemsInFolder ${totalItemsInFolder <= 1 ? 'lieux' : 'lieux'}";
      }
      ...
      if (isExpanded) {
        subFolders.forEach((subTitle, items) {
          ...
          final String countLabel = mainTitle == "Infos Pratiques"
              ? "$count ${count <= 1 ? 'infos' : 'infos'}"
              : "$count ${count <= 1 ? 'lieux' : 'lieux'}";
          ...
          if (isSubExpanded) {
            if (items.isNotEmpty) {
              ...
              list.add(Padding(
                ...
                child: Text(mainTitle == "Infos Pratiques" ? "Aucune infos enregistrée" : "Aucun lieu enregistré", style: GoogleFonts.montserrat(color: Colors.white24, fontSize: 11, fontStyle: FontStyle.italic)),
              ));
            }
          }
        });
